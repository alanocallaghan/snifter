...
The main functionality of the package lies in the `fi_tsne` function.
This function returns a matrix of t-SNE co-ordinates. In this case,
...
```{r run}
mat <- reducedDim(data)
snifter <- fi_tsne(mat, random_state = 42L)
plot(snifter,
    pch = 19,
    xlab = "t-SNE 1", ylab = "t-SNE 2",
    col = as.numeric(data$level1class)
)
legend(40, 50,
    legend = levels(data$level1class),
    col = seq_len(nlevels(data$level1class)),
    pch = 19
)
```
...
To do this, the option `as_matrix = FALSE` needs to be specified
when calling `fi_tsne`. Here, we will split the data into "training"
...
embedding <- fi_tsne(train_mat, as_matrix = FALSE, random_state = 42L)
```

The `embedding` object is of class `"openTSNE.tsne.TSNEEmbedding"`
(it also inherits from `"numpy.ndarray"`). To access the numeric 
t-SNE co-ordinates, we can simply call `as.matrix`. 
Following generation of the embedding, we can now project the unseen test
data into this t-SNE embedding.

```{r plot-embed}
embedding_coords <- as.matrix(embedding)
new_coords <- embed(embedding, test_mat)
plot(
    embedding_coords,
    pch = 19,
    col = as.numeric(train_label),
    xlab = "t-SNE 1", ylab = "t-SNE 2",
)
legend(30, 50,
    legend = levels(data$level1class),
